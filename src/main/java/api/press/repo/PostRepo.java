...
public class PostRepo {
    ...
    public Optional<Post> insert(Post post){
        ...
        try {
                ...
                jdbcTemplate.update(con -> QueryUtil.getInsertStat(con, statement, List.of(post.getEditorID(), post.getEditorName(), post.getTitle(), post.getBody(), post.getNo_views(),
                        post.getNo_likes(), post.getNo_dislikes(), post.getCreate_date(), post.isState(), post.getPostType().label)), holder);
                post.setId(Objects.requireNonNull(holder.getKey()).intValue());
